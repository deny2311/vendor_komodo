...
#           (C) 2020-2021 The KomodoOS Project
#
...
function setup_vendor() {
    ...
    export KOMODO_ROOT="$3"
    if [ ! -d "$KOMODO_ROOT" ]; then
        echo "\$KOMODO_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$KOMODO_ROOT/$OUTDIR" ]; then
        mkdir -p "$KOMODO_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$KOMODO_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$KOMODO_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$KOMODO_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$KOMODO_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2017 The KomodoOS Project\n" >> $1
            ...
            printf "# Copyright (C) 2017-$YEAR The KomodoOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The LineageOS Project\n" >> $1
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The KomodoOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The LineageOS Project\n" > $1
        printf "# Copyright (C) $YEAR The KomodoOS Project\n" > $1
...
function oat2dex() {
    ...
    if [ ! -f "$KOMODO_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$KOMODO_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$KOMODO_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$KOMODO_ROOT"/vendor/ancient/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$KOMODO_ROOT"/"$OUTDIR"/radio
