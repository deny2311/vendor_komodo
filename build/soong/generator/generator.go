...
func init() {
	android.RegisterModuleType("komodo_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = komodoExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = komodoExpandVariables(ctx, dep_file)
	...
	cmd := komodoExpandVariables(ctx, String(g.properties.Cmd))
